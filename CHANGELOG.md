...
### Fixed
- Fixed automatic notification settings not persisting when unchecked: cast `get_option()` return to boolean so `checked()` correctly renders unchecked state
...
### Security
- Fixed SQL injection vulnerability in admin notifications list table: replaced raw `$_REQUEST['orderby']` interpolation with an explicit whitelist map to prevent taint flow into database queries
